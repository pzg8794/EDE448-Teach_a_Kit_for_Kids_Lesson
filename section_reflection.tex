...
\end{tikzpicture}}
\caption{Neurodivergent access map. Participation is interpreted through communication, sensory access, processing and pacing, and peer belonging, not through behavior alone.}
\label{fig:access-map}
\end{figure}

Figure~\ref{fig:access-map} captures the central reflection: behavior cannot be understood apart from access. A delayed answer may indicate processing time or another communication mode. Moving away may indicate sensory load. Difficulty joining a coding task may reflect unclear steps. A quiet or new student outside the group may need an explicit invitation and a safe role, not pressure.
